...
NEW_PATH = 'data/New_Datasets/Resultados_2.csv'
LIST_PATH = 'data/New_Datasets/Resultados2_lista.csv'
...
def mutation5_procedure(children_list, additional_undesired_ingredients):
    children_list = list(children_list)
    # Asegurarse de que los additional_undesired_ingredients no sean elegidos para la mezcla
    full_list = children_list + additional_undesired_ingredients
    
    if len(children_list) > 2:
        # Seleccionar índices aleatorios para la sublista, asegurándose de que solo se seleccionen de children_list
        start_index = np.random.randint(0, len(children_list) - 2)
        end_index = np.random.randint(start_index + 1, len(children_list))
        
        # Extraer la sublista de children_list, no de full_list
        sublist = children_list[start_index:end_index]
        
        # Mezclar la sublista
        np.random.shuffle(sublist)
        
        # Reinsertar la sublista mezclada en children_list
        new_children_list = children_list[:start_index] + sublist + children_list[end_index:]
    else:
        # Si la lista es demasiado corta para mezclar, se devuelve sin cambios
        new_children_list = children_list
...
def genetic_algorithm(
    ...
    for gen in range(generations):
        try:
            ...
            mutations5 = childrens.apply(
                mutation5_procedure,
                additional_undesired_ingredients=undesired_ingredients
            )

            children_with_mutations = pd.concat(
                [mutations1, mutations2, mutations3, mutations4, mutations5]).reset_index(
                drop=True)
...
def process_results(results):
    ...
    df = pd.DataFrame(final_result.iloc[0], columns=['Ingredient'])
    print(f'Saving target, at {csv_path}')
    save_as_csv(what=df, where=csv_path)
